"""
differential_robot_with_wheels_optimized.py

Robô diferencial com rodas físicas (joints) em PyBullet, sensores ultrassônicos (ray tests),
controle PID simples, algoritmo local de evasão, log de trajetória, mapa de ocupação
e otimização por repetição.
"""
...
# -----------------------
# Mapa de ocupação com tempo por célula
# -----------------------
class CoverageMap:
...
def build_environment():
    ...
    def add_wall(center,half_extents):
        ...
        body = p.createMultiBody(baseMass=0,baseCollisionShapeIndex=col,baseVisualShapeIndex=vis,basePosition=center)
        return body
...
# Loop principal
# -----------------------
def run_simulation_multi(executions=3):
    ...
    for run_idx in range(executions):
        ...
        all_trajectories.append((traj_x,traj_y))
        efficiency_history.append(coverage_map.coverage_ratio())
    ...
    plt.plot(range(1,executions+1),[e*100 for e in efficiency_history],marker='o')
    plt.xlabel("Execução")
    plt.ylabel("Cobertura (%)")
    plt.title("Evolução da eficiência da limpeza")
    plt.grid(True)
    plt.savefig("eficiencia_limpeza.png", dpi=150)

    # -----------------------
    # Gráfico da trajetória
    # -----------------------
    plt.figure(figsize=(8,6))
    for traj_x, traj_y in all_trajectories:
    ...
    plt.axis("equal")
    plt.grid(True)
    plt.savefig("caminho_robo.png", dpi=150)
