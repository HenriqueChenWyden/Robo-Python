...
class DifferentialRobot:
    def __init__(self, physics_client, start_pos=(0,0,ROBOT_HEIGHT/2), start_yaw=0.0):
        ...
        body_collision = p.createCollisionShape(p.GEOM_CYLINDER, radius=ROBOT_RADIUS, height=ROBOT_HEIGHT)
        body_visual = p.createVisualShape(p.GEOM_CYLINDER, radius=ROBOT_RADIUS, length=ROBOT_HEIGHT, rgbaColor=[0.2,0.6,0.9,1.0])
        ...
        link_positions = [
            [0.0, self.wheel_base/2.0, -ROBOT_HEIGHT/2],
            [0.0, -self.wheel_base/2.0, -ROBOT_HEIGHT/2],
            [ROBOT_RADIUS, 0.0, -ROBOT_HEIGHT/2]
        ]
        link_orientations = [
        ...
        self.body = p.createMultiBody(
            ...
            linkMasses=[0.3,0.3,0.1], linkCollisionShapeIndices=[wheel_col,wheel_col,wheel_col],
            linkVisualShapeIndices=[wheel_vis,wheel_vis,wheel_vis], linkPositions=link_positions,
            linkOrientations=link_orientations, linkInertialFramePositions=[[0,0,0],[0,0,0],[0,0,0]],
            linkInertialFrameOrientations=[p.getQuaternionFromEuler([0,0,0]),p.getQuaternionFromEuler([0,0,0]),p.getQuaternionFromEuler([0,0,0])],
            linkParentIndices=[0,0,0], linkJointTypes=[p.JOINT_REVOLUTE,p.JOINT_REVOLUTE,p.JOINT_REVOLUTE],
            linkJointAxis=[[0,1,0],[0,1,0],[0,1,0]]
        )

        self.left_wheel_idx=0
        self.right_wheel_idx=1
        self.front_wheel_idx=2
        ...
        # roda frontal passiva (estabilizadora) - livre para girar
        p.setJointMotorControl2(self.body,self.front_wheel_idx,p.VELOCITY_CONTROL,targetVelocity=0,force=0)
...
def run_simulation_node_red(executions=3):
    ...
    for run_idx in range(executions):
        ...
        robot = DifferentialRobot(physicsClient, start_pos=(-1.0,-0.6,ROBOT_HEIGHT/2), start_yaw=math.radians(30))
        ...
        while sim_time<SIM_DURATION:
            ...
            if remaining_dirt:
                ...
                for d in list(remaining_dirt):
                    ...
                    if dist < best_d:
                        ...
                        best = (d, dpos)
                if best is not None:
                    controller.target_point = (best[1][0], best[1][1])
                else:
                    controller.target_point = None
            else:
                controller.target_point = None
            ...
            # verificar se passou por cima de algum ponto de sujeira
            for d in list(remaining_dirt):
                try:
                    dpos, _ = p.getBasePositionAndOrientation(d)
                except Exception:
